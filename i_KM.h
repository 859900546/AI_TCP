...
public:
	...
	BOOL OpenDevice(void);//连接默认VID/PID的设备
	BOOL OpenDeviceByID(SHORT vid, SHORT pid);//连接任意VID/PID的设备
	SHORT GetVID() const;
	SHORT GetPID() const;
	...
	void MouseButtonDown(BYTE index);//鼠标键按下，参数1-8
	void MouseButtonUp(BYTE index);//鼠标键弹起，参数1-8
	void MouseButtonClick(BYTE index,int min=0,int max=0);//鼠标键点击，参数1-8。min和max代表随机延时区间
